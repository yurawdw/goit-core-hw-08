"""
Address Book Assistant - v0.0.15
...
def greeting():
    ...
    print(f"{Fore.GREEN}{GREETING_STR}{Style.RESET_ALL}")
    print(HELP, "\n")
...
def show_birthday(args, book):
    ...
    if record and record.birthday:
        return f"\n{Fore.WHITE}Birthday: {Fore.GREEN}{record.birthday.birthday.strftime('%d.%m.%Y')}{Style.RESET_ALL}\n"
    ...
    try:
        ...
        for record in book.data.values():
            if record.birthday:
                ...
                birthday = birthday + shift_if_weekend(birthday)
                if 0 <= (birthday - today).days <= days:
                    upcoming_birthdays.append({"name" : str(record.name), "birthday" : birthday.strftime('%d.%m.%Y')})
        if upcoming_birthdays:
            return upcoming_birthdays
        raise ValueError(f"{Fore.YELLOW}No upcoming birthdays in the next {days} days.{Style.RESET_ALL}")
...
def main():
    ...
    while True:
        ...
        if command in ["close", "exit", "e", "c"]:
            ...
            upcoming_birthdays = get_upcoming_birthdays(args, book)
            if upcoming_birthdays:
                print(f"\n{Fore.GREEN}Upcoming birthdays:{Style.RESET_ALL}\n" + f"\n{'\n'.join([f"{Fore.GREEN}{birthday['name']} {Fore.WHITE}will celebrate birthday on {Fore.YELLOW}{birthday['birthday']}{Style.RESET_ALL}" for birthday in get_upcoming_birthdays(args, book)])}\n" + f"{Style.RESET_ALL}\n")
